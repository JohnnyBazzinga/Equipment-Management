...
if(isset($_POST["submitEquipamentos"])){ // Procurar Equipamento específico
 ?>
<!DOCTYPE html>
<html>
          ...
          <table id="equipamentos" class="table table-striped table-bordered" style="width:100%">
            <thead>
              <tr>
                ...
                <th>Editar Informações</th>
              </tr>
            </thead>
            <tbody>
              <?php
                         $stmt = $DB->prepare("SELECT tipoProduto.tipo, `numeroProduto`, `designacao`, `marca`, `composto`, 
                                                              produto.`codTipoProduto`, `numeroSala`,escola, `serie`, `obs`, data FROM `produto` 
                                                              INNER JOIN tipoProduto ON produto.codTipoProduto = tipoProduto.codTipoProduto 
                                                              WHERE composto=0 and `numeroSala`=? and produto.codTipoProduto=? and escola=?");
                         $stmt->bind_param("sss", $_POST['salas'], $_POST['codTipoProduto'],$_POST['escolas']);
                         $stmt->execute();
                        
                         $stmt->bind_result($tipoProduto, $numeroProduto, $designacao, $marca, $composto, 
                                            $codTipoProduto, $numeroSala,$escola, $serie, $obs, $data);
                         while ($stmt->fetch())
                           if ($codTipoProduto == 3) {
